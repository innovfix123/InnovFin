/**
 * Token endpoint for the Google Drive resource. Mirror of gateway's, off the DRIVE config/grants so the
 * minted access token's audience = this resource. Reuses the shared (resource-agnostic) JWT verifier,
 * PKCE check, and CORS helpers. Public clients only (no secret).
 */
import { verifyJwt } from "@/mcp/onlycare-tds/oauth/tokens";
import { verifyPkce } from "@/mcp/onlycare-tds/oauth/pkce";
import { issueTokens, claimCode } from "@/mcp/drive/oauth/grants";
import { isMcpAllowed, resource } from "@/mcp/drive/oauth/config";
