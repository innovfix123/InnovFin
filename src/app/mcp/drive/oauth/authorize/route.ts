/**
 * Authorization endpoint for the Google Drive resource (OAuth 2.1 authorization-code + PKCE).
 * Mirror of gateway's authorize route, off the DRIVE config (so the code's audience = this resource).
 *
...
import { origin, resource, isMcpAllowed, CODE_TTL_SEC } from "@/mcp/drive/oauth/config";
...
export async function GET(req: Request): Promise<Response> {
  ...
  if (!isMcpAllowed(email)) {
    return back({ error: "access_denied", error_description: `${email} is not authorized for the Drive MCP. Ask an admin to add you.` });
  }
