/**
 * Networked Google Drive MCP endpoint — https://gst.innovfix.ai/mcp/drive
 *
 * Served by the innovfin Next.js app (pm2 :3000, behind nginx TLS). Mirror of the Gateway endpoint
 * (src/app/mcp/gateway-settlements/route.ts) — same stateless Streamable HTTP transport, same dual auth
 * (a static per-user bearer token OR an OAuth access token from the native Claude connector), same
 * per-call access log (audit.ts) — but bound to the DRIVE resource + allowlist, so all MCP endpoints
 * stay fully independent.
 *
 * Inbound auth (who may call this MCP) is enforced here; the OUTBOUND Google service-account credential
 * lives entirely inside the Drive client and is never exposed to callers.
 *
...
import { buildDriveServer } from "@/mcp/drive/factory";
import { authenticate, bearerFromRequest, hasConfiguredTokens } from "@/mcp/drive/http-auth";
import { authenticateOAuth } from "@/mcp/drive/oauth/grants";
import { protectedResourceMetadataUrl } from "@/mcp/drive/oauth/config";
import { envVar } from "@/mcp/drive/env";
import { audit, safeArgs } from "@/mcp/drive/audit";

export const runtime = "nodejs";        // needs node crypto, fs (audit), and outbound fetch to Google
export const dynamic = "force-dynamic"; // never cache; every call is authed + audited + live
...
export async function POST(req: Request): Promise<Response> {
  ...
  if (!hasConfiguredTokens() && !envVar("AUTH_SECRET")) {
    audit({ ts: new Date(started).toISOString(), event: "error", user: "anonymous", ip, status: 503, ms: Date.now() - started, reason: "no DRIVE_MCP_TOKEN_* and no AUTH_SECRET configured" });
