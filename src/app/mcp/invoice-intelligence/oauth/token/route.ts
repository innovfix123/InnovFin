/**
 * Token endpoint for the invoice-intelligence resource. Mirror of Hima's, off this config/grants so
 * the minted access token's audience = this resource. Reuses the shared (resource-agnostic) JWT
...
import { issueTokens, claimCode } from "@/mcp/invoice-intelligence/oauth/grants";
import { isMcpAllowed, resource } from "@/mcp/invoice-intelligence/oauth/config";
...
export async function POST(req: Request): Promise<Response> {
  ...
  if (grantType === "authorization_code") {
    ...
    // Shared signing key across MCPs → pin the code to THIS resource (reject a code minted elsewhere).
