/**
 * Networked TDS Working MCP endpoint — https://gst.innovfix.ai/mcp/tds-working
 *
 * Served by the innovfin Next.js app (pm2 :3000, behind nginx TLS). Mirror of the Gateway Settlements
 * endpoint (src/app/mcp/gateway-settlements/route.ts) — same stateless Streamable HTTP transport, same
 * dual auth (a static per-user bearer token OR an OAuth access token from the native Claude
 * connector), same per-call access log (audit.ts) — but bound to the TDS_WORKING resource + allowlist,
 * so all four MCP endpoints (Only Care / Hima / Gateway Settlements / TDS Working) stay fully
 * independent. This is InnovFix's internal register (salary; directors/freelancers to follow), not a
 * per-app payout MCP.
 *
...
import { buildTdsWorkingServer } from "@/mcp/tds-working/factory";
import { authenticate, bearerFromRequest, hasConfiguredTokens } from "@/mcp/tds-working/http-auth";
import { authenticateOAuth } from "@/mcp/tds-working/oauth/grants";
import { protectedResourceMetadataUrl } from "@/mcp/tds-working/oauth/config";
import { envVar } from "@/mcp/tds-working/env";
import { audit, safeArgs } from "@/mcp/tds-working/audit";

export const runtime = "nodejs";        // needs node crypto and fs (audit)
...
export async function POST(req: Request): Promise<Response> {
  ...
  if (!hasConfiguredTokens() && !envVar("AUTH_SECRET")) {
    audit({ ts: new Date(started).toISOString(), event: "error", user: "anonymous", ip, status: 503, ms: Date.now() - started, reason: "no TDS_WORKING_MCP_TOKEN_* and no AUTH_SECRET configured" });
