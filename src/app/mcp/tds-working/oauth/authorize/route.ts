/**
 * Authorization endpoint for the TDS Working resource (OAuth 2.1 authorization-code + PKCE).
 * Mirror of Gateway Settlements' authorize route, off the TDS_WORKING config (so the code's audience
 * = this resource).
 *
...
import { origin, resource, isMcpAllowed, CODE_TTL_SEC } from "@/mcp/tds-working/oauth/config";
...
export async function GET(req: Request): Promise<Response> {
  ...
  if (!isMcpAllowed(email)) {
    return back({ error: "access_denied", error_description: `${email} is not authorized for TDS Working. Ask an admin to add you.` });
  }
