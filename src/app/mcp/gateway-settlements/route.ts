/**
 * Networked Gateway Settlements MCP endpoint — https://gst.innovfix.ai/mcp/gateway-settlements
 *
 * Served by the innovfin Next.js app (pm2 :3000, behind nginx TLS). Mirror of the Hima endpoint
 * (src/app/mcp/hima-tds/route.ts) — same stateless Streamable HTTP transport, same dual auth (a static
 * per-user bearer token OR an OAuth access token from the native Claude connector), same per-call
 * access log (audit.ts) — but bound to the GATEWAY resource + allowlist, so all three MCP endpoints
 * (Only Care / Hima / Gateway Settlements) stay fully independent.
 *
...
import { buildGatewaySettlementsServer } from "@/mcp/gateway-settlements/factory";
import { authenticate, bearerFromRequest, hasConfiguredTokens } from "@/mcp/gateway-settlements/http-auth";
import { authenticateOAuth } from "@/mcp/gateway-settlements/oauth/grants";
import { protectedResourceMetadataUrl } from "@/mcp/gateway-settlements/oauth/config";
import { envVar } from "@/mcp/gateway-settlements/env";
import { audit, safeArgs } from "@/mcp/gateway-settlements/audit";

export const runtime = "nodejs";        // needs node crypto, fs (audit), and outbound fetch to gateways
...
export async function POST(req: Request): Promise<Response> {
  ...
  if (!hasConfiguredTokens() && !envVar("AUTH_SECRET")) {
    audit({ ts: new Date(started).toISOString(), event: "error", user: "anonymous", ip, status: 503, ms: Date.now() - started, reason: "no GATEWAY_MCP_TOKEN_* and no AUTH_SECRET configured" });
