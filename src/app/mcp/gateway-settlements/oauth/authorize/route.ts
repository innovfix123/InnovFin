/**
 * Authorization endpoint for the Gateway Settlements resource (OAuth 2.1 authorization-code + PKCE).
 * Mirror of Hima's authorize route, off the GATEWAY config (so the code's audience = this resource).
 *
 * Identity reuses the portal login: no valid session → bounce to /login with ?next back here; once
 * signed in, check the allowlist and issue a short-lived, PKCE-bound authorization code, then redirect
 * to the client's registered redirect_uri. The JWT signer + stateless client parser are the
 * resource-agnostic ones shared with Only Care.
 */
import { getSessionEmail } from "@/lib/session";
import { parseClient } from "@/mcp/onlycare-tds/oauth/clients";
import { signJwt } from "@/mcp/onlycare-tds/oauth/tokens";
import { origin, resource, isMcpAllowed, CODE_TTL_SEC } from "@/mcp/gateway-settlements/oauth/config";
...
export async function GET(req: Request): Promise<Response> {
  ...
  if (!isMcpAllowed(email)) {
    return back({ error: "access_denied", error_description: `${email} is not authorized for Gateway Settlements. Ask an admin to add you.` });
  }
