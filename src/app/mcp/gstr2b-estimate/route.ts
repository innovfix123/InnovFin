/**
 * Networked Estimated GSTR-2B MCP endpoint — https://gst.innovfix.ai/mcp/gstr2b-estimate
 *
 * Served by the innovfin Next.js app (pm2 :3000, behind nginx TLS). Mirror of the TDS Working
 * endpoint (src/app/mcp/tds-working/route.ts) — same stateless Streamable HTTP transport, same dual
 * auth (a static per-user bearer token OR an OAuth access token from the native Claude connector),
 * same per-call access log (audit.ts) — but bound to the GSTR2B_EST resource + allowlist, so all
 * the MCP endpoints stay fully independent. Data comes from the invoice-intelligence registry via
 * its localhost MCP (pm2 invoice-intel-mcp on 127.0.0.1:8765) — this endpoint owns no store.
 *
...
import { buildGstr2bEstimateServer } from "@/mcp/gstr2b-estimate/factory";
import { authenticate, bearerFromRequest, hasConfiguredTokens } from "@/mcp/gstr2b-estimate/http-auth";
import { authenticateOAuth } from "@/mcp/gstr2b-estimate/oauth/grants";
import { protectedResourceMetadataUrl } from "@/mcp/gstr2b-estimate/oauth/config";
import { envVar } from "@/mcp/gstr2b-estimate/env";
import { audit, safeArgs } from "@/mcp/gstr2b-estimate/audit";

export const runtime = "nodejs";        // needs node crypto and fs (audit, workbook drop folder)
...
export async function POST(req: Request): Promise<Response> {
  ...
  if (!hasConfiguredTokens() && !envVar("AUTH_SECRET")) {
    audit({ ts: new Date(started).toISOString(), event: "error", user: "anonymous", ip, status: 503, ms: Date.now() - started, reason: "no GSTR2B_EST_MCP_TOKEN_* and no AUTH_SECRET configured" });
...
// Stateless mode exposes no server-initiated stream and no session to delete. Require a valid token
