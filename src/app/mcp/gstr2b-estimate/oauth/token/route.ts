/**
 * Token endpoint for the Estimated GSTR-2B resource. Mirror of TDS Working's, off the GSTR2B_EST
 * config/grants so the minted access token's audience = this resource. Reuses the shared
 * (resource-agnostic) JWT verifier, PKCE check, and CORS helpers. Public clients only (no secret).
 */
import { verifyJwt } from "@/mcp/onlycare-tds/oauth/tokens";
import { verifyPkce } from "@/mcp/onlycare-tds/oauth/pkce";
import { issueTokens, claimCode } from "@/mcp/gstr2b-estimate/oauth/grants";
import { isMcpAllowed, resource } from "@/mcp/gstr2b-estimate/oauth/config";
...
export async function POST(req: Request): Promise<Response> {
  ...
  if (grantType === "authorization_code") {
    ...
    // Share the signing key with the other resources, so pin the code to THIS resource (reject a code
    // minted for a different resource that happens to verify under the same key).
