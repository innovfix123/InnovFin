/**
 * Authorization endpoint for the Estimated GSTR-2B resource (OAuth 2.1 authorization-code + PKCE).
 * Mirror of TDS Working's authorize route, off the GSTR2B_EST config (so the code's audience =
 * this resource).
 *
...
import { origin, resource, isMcpAllowed, CODE_TTL_SEC } from "@/mcp/gstr2b-estimate/oauth/config";
...
export async function GET(req: Request): Promise<Response> {
  ...
  if (!isMcpAllowed(email)) {
    return back({ error: "access_denied", error_description: `${email} is not authorized for the Estimated GSTR-2B MCP. Ask an admin to add you.` });
  }
