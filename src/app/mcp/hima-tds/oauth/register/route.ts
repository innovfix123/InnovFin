/**
 * Dynamic Client Registration endpoint (RFC 7591) for the Hima resource. DCR is resource-agnostic
 * (the client_id just encodes the client's redirect_uris), so this reuses the shared, stateless
 * client + CORS helpers. Public PKCE clients only (no secret issued). Returns 201 with a client_id.
 */
...
export async function POST(req: Request): Promise<Response> {
  ...
  // Echo the registered metadata back (RFC 7591 §3.2.1), public client, no secret.
  return corsJson(
    {
      client_id,
