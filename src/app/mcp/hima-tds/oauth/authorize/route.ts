/**
 * Authorization endpoint for the Hima resource (OAuth 2.1 authorization-code + PKCE). Mirror of
 * Only Care's authorize route, off the HIMA config (so the code's audience = the Hima resource).
 *
 * Identity reuses the portal login: if there's no valid session cookie we bounce to /login with a
 * ?next back here; once signed in, we check the Hima allowlist and issue a short-lived, PKCE-bound
 * authorization code, then redirect to the client's registered redirect_uri. The JWT signer and the
 * stateless client parser are the resource-agnostic ones shared with Only Care.
 */
import { getSessionEmail } from "@/lib/session";
import { parseClient } from "@/mcp/onlycare-tds/oauth/clients";
import { signJwt } from "@/mcp/onlycare-tds/oauth/tokens";
import { origin, resource, isMcpAllowed, CODE_TTL_SEC } from "@/mcp/hima-tds/oauth/config";
...
export async function GET(req: Request): Promise<Response> {
  ...
  if (!isMcpAllowed(email)) {
    return back({ error: "access_denied", error_description: `${email} is not authorized for Hima TDS. Ask an admin to add you.` });
  }
