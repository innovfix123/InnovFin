/**
 * Networked Hima TDS MCP endpoint — https://gst.innovfix.ai/mcp/hima-tds
 *
 * Served by the innovfin Next.js app (pm2 :3000, behind nginx TLS for gst.innovfix.ai).
 * Mirror of the Only Care endpoint (src/app/mcp/onlycare-tds/route.ts) — same stateless Streamable
 * HTTP transport, same dual auth (a static per-user bearer token, OR an OAuth access token from the
 * native Claude connector), same per-call access log (see audit.ts) — but bound to the HIMA
 * resource + allowlist, so the two endpoints stay fully independent.
 *
...
import { buildHimaServer } from "@/mcp/hima-tds/factory";
import { authenticate, bearerFromRequest, hasConfiguredTokens } from "@/mcp/hima-tds/http-auth";
import { authenticateOAuth } from "@/mcp/hima-tds/oauth/grants";
import { protectedResourceMetadataUrl } from "@/mcp/hima-tds/oauth/config";
import { envVar } from "@/mcp/hima-tds/env";
import { audit, safeArgs } from "@/mcp/hima-tds/audit";
...
export async function POST(req: Request): Promise<Response> {
  ...
  if (!hasConfiguredTokens() && !envVar("AUTH_SECRET")) {
    audit({ ts: new Date(started).toISOString(), event: "error", user: "anonymous", ip, status: 503, ms: Date.now() - started, reason: "no HIMA_MCP_TOKEN_* and no AUTH_SECRET configured" });
