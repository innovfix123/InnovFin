/**
 * Token issuance + access-token verification for the Hima MCP route. Mirrors Only Care's
 * oauth/grants.ts but binds tokens to the HIMA resource audience and re-checks the HIMA allowlist.
 *
...
/** Mint an access+refresh pair for an authorized user, audience-bound to the HIMA resource (RFC 8707). */
...
/**
 * Verify a bearer access token for the Hima MCP route: valid signature, not expired, audience ==
 * the HIMA resource, and the subject is still allowlisted. Returns the principal (email) or null.
 * The audience check is what keeps an Only Care token from ever working here (and vice-versa).
 */
...
// Best-effort single-use for authorization codes (own set, independent of Only Care's).
