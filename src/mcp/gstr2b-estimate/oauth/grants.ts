/**
 * Token issuance + access-token verification for the Estimated GSTR-2B MCP route. Mirrors the other
 * MCPs' oauth/grants.ts but binds tokens to the GSTR2B_EST resource audience and re-checks its
 * allowlist.
 *
...
/** Mint an access+refresh pair for an authorized user, audience-bound to the GSTR2B_EST resource (RFC 8707). */
...
/**
 * Verify a bearer access token for the Estimated GSTR-2B route: valid signature, not expired,
 * audience == this resource, and the subject is still allowlisted. The audience check is what keeps
 * an Only Care / Hima / Gateway / TDS Working token from ever working here (and vice-versa).
 */
