...
/** Repo root, derived from this module's location (src/mcp/gstr2b-estimate → ../../..). */
export const REPO_ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "../../..");

/**
 * Read a key from process.env, falling back to parsing the repo `.env` — so the MCP server runs
 * standalone (via tsx) without a dotenv dependency. Mirrors src/mcp/tds-working/env.ts.
 */
 ...
 * Used to discover per-user MCP tokens (GSTR2B_EST_MCP_TOKEN_<USER>) without a hardcoded user list.
 */
