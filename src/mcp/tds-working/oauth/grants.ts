/**
 * Token issuance + access-token verification for the TDS Working MCP route. Mirrors Gateway
 * Settlements' oauth/grants.ts but binds tokens to the TDS_WORKING resource audience and re-checks
 * its allowlist.
 *
...
/** Mint an access+refresh pair for an authorized user, audience-bound to the TDS_WORKING resource (RFC 8707). */
...
/**
 * Verify a bearer access token for the TDS Working route: valid signature, not expired, audience ==
 * the TDS_WORKING resource, and the subject is still allowlisted. The audience check is what keeps
 * an Only Care / Hima / Gateway token from ever working here (and vice-versa).
 */
