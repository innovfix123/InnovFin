/**
 * OAuth server configuration for the TDS Working MCP resource.
 *
 * Like Hima and Gateway Settlements (and UNLIKE Only Care), the issuer is the resource URL itself — a
 * co-located AS on the /mcp/tds-working path, NOT the root origin. Only Care claims the ROOT
 * well-known AS-metadata path (single-valued/global); this endpoint uses a path-based issuer
 * discovered by RFC 8414 path-insertion, so all four OAuth servers stay fully independent and each
 ...
 * TDS_WORKING_MCP_ALLOWED_EMAILS + `pm2 restart innovfin` (per-user, no token store).
 */
import { envVar } from "../env";

/** Public origin the endpoint is served from. Override per-env with TDS_WORKING_MCP_ORIGIN. */
export function origin(): string {
  return (envVar("TDS_WORKING_MCP_ORIGIN") ?? "https://gst.innovfix.ai").replace(/\/$/, "");
}

/** The MCP resource identifier (RFC 8707 audience). */
export function resource(): string {
  return `${origin()}/mcp/tds-working`;
}
/** OAuth issuer = the resource URL (co-located AS, path-based). AS metadata is discovered by RFC 8414
 *  path-insertion at /.well-known/oauth-authorization-server/mcp/tds-working. */
...
export const oauthPaths = {
  authorize: "/mcp/tds-working/oauth/authorize",
  token: "/mcp/tds-working/oauth/token",
  register: "/mcp/tds-working/oauth/register",
} as const;
...
export function protectedResourceMetadataUrl(): string {
  return `${origin()}/.well-known/oauth-protected-resource/mcp/tds-working`;
}
...
/** Emails permitted to use the TDS Working MCP (defaults to JP/Shoyab/Fida — same finance team). */
export function allowedEmails(): string[] {
  const env = envVar("TDS_WORKING_MCP_ALLOWED_EMAILS");
  const list = env
    ? env.split(",")
