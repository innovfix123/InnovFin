...
/** Repo root, derived from this module's location (src/mcp/tds-working → ../../..). */
export const REPO_ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "../../..");

/**
 * Read a key from process.env, falling back to parsing the repo `.env` — so the MCP server runs
 * standalone (via tsx) without a dotenv dependency. Mirrors src/mcp/gateway-settlements/env.ts.
 */
 ...
 * Used to discover per-user MCP tokens (TDS_WORKING_MCP_TOKEN_<USER>) without a hardcoded user list.
 */
