/**
 * Access/audit log for the networked TDS Working endpoint. Mirror of Gateway/Hima/Only Care's
 * audit.ts, writing to tds-working-mcp/logs/access.jsonl.
 *
 ...
 *
 * We record the *shape* of a call (tool name, period, employee filter) — never salary amounts,
 * PANs, or token secrets.
 */
import { appendFileSync, mkdirSync } from "node:fs";
import { resolve } from "node:path";
import { REPO_ROOT } from "./env";

const LOG_DIR = resolve(REPO_ROOT, "tds-working-mcp/logs");
...
/**
 * Reduce raw tool arguments to a summary safe to persist: period (a month string) and whether an
 * employee filter was supplied (never the filter text or any amount).
 */
export function safeArgs(args: unknown): Record<string, unknown> | undefined {
  ...
  if (typeof a.employee === "string") out.hasEmployeeFilter = true;
...
export function audit(entry: AuditEntry): void {
  ...
  try {
    ...
    console.error("tds-working audit: file append failed:", e);
  }
  console.error("tds-working audit", line);
}
