/**
 * Bearer-token auth for the networked invoice-intelligence endpoint. Mirror of Hima's http-auth.ts
 * with the INVOICE_INTEL_MCP_TOKEN_<USER> prefix.
 *
 * Each user gets their own secret in `.env` as INVOICE_INTEL_MCP_TOKEN_<USER> (e.g. ..._FIDA, ..._JP).
 * Tokens are per-user and independently revocable: delete one user's line + `pm2 restart innovfin`
 * and only that user loses access. This endpoint reads company invoice data — no shared token, ever.
 */
import { createHash, timingSafeEqual } from "node:crypto";
import { envVarsWithPrefix } from "./env";

const PREFIX = "INVOICE_INTEL_MCP_TOKEN_";

export interface Principal {
  /** Stable user label, derived from the env-var suffix (e.g. "FIDA", "JP"). */
  user: string;
